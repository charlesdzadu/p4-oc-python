...
class MatchView:
    ...
    def update_single_match_menu(self, match: Match):
        ...
        print(f"\n{bcolors.OKGREEN}Match {match.id} : {bcolors.ENDC}")
        print(
            f"{match.player_1.first_name} {match.player_1.last_name} - vs "
            f"{match.player_2.first_name} {match.player_2.last_name}"
        )
        ...
        message = f"Le match {match.id} a été mis à jour avec le score {match.score_player_1} - {match.score_player_2}"
