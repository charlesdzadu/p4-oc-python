        ...
        try:
            ...
            if type == datetime.date or type == datetime.datetime:
                try:
                    ...
                    if min_value is not None and value < min_value:
                        print(
                            f"{
                                bcolors.FAIL} Erreur: La date doit être le {
                                min_value.strftime(date_format)} ou après.{
                                bcolors.ENDC}")
                        continue
                    ...
                    if max_value is not None and value > max_value:
                        print(
                            f"{
                                bcolors.FAIL} Erreur: La date doit être le {
                                max_value.strftime(date_format)} ou avant.{
                                bcolors.ENDC}")
                        continue
                    ...
                    print(
                        f"{bcolors.FAIL} Erreur: La date doit être valide dans le format {date_format}. "
                        f"Veuillez réessayer.{bcolors.ENDC}"
                    )
                    continue
            ...
            if type == str:
                ...
                if min_value is not None and len(value) < min_value:
                    print(
                        f"{bcolors.FAIL} Erreur: La chaîne doit contenir au moins {min_value} "
                        f"caractères.{bcolors.ENDC}"
                    )
                    continue

                if max_value is not None and len(value) > max_value:
                    print(
                        f"{bcolors.FAIL} Erreur: La chaîne doit contenir au plus {max_value} caractères.{bcolors.ENDC}"
                    )
                    continue
            ...
            print(
                f"{bcolors.FAIL} Erreur: La valeur doit être valide dans le format "
                f"{type.__name__}. Veuillez réessayer.{bcolors.ENDC}"
            )
